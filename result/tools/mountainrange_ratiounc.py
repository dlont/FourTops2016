...
def main(arguments):
	...
	if arguments.ratio_reference is not None:
		...
		for hname in jsondic['reference']:
			...
			for ibin in range(1,h.GetNbinsX()+1):
				longhist.SetBinContent(xmin+ibin,0)
				longhist.SetBinError(xmin+ibin,h.GetBinError(ibin)/h.GetBinContent(ibin))
			xmin += h.GetNbinsX()
		longhist_rat = longhist
		longhist_rat.SetMarkerStyle(0)

	#prefit
	longhistfromfiles_pre = []
	for item,f in enumerate(inputrootfiles):
		longhist = rt.TH1D("prefit_"+f.GetName(),"",nbins,0.,rangex)
		longhist.SetMarkerStyle(1)
		...
		for hname in jsondic['prefitbg']:
			h = f.Get(hname)
			for ibin in range(1,h.GetNbinsX()+1):
				longhist.SetBinContent(xmin+ibin,h.GetBinError(ibin)/h.GetBinContent(ibin))
			xmin += h.GetNbinsX()
	...
	longhist_rat.Draw("hist")
	longhist_rat.GetYaxis().SetLimits(-0.3,0.3)
	longhist_rat.GetYaxis().SetRangeUser(-0.3,0.3)
	for h in longhistfromfiles_post:
		h.Draw("same hist E1")

	leg = rt.TLegend(0.21,0.7,0.89,0.89)
	leg.SetBorderSize(0)
	leg.SetNColumns(2)
	for h in longhistfromfiles_post: leg.AddEntry(h,h.GetName(),'l')
	leg.Draw()

	if 'filename' in jsondic: c.Print(arguments.dir+'/'+jsondic['filename']+'_uncratio.'+arguments.extension)
