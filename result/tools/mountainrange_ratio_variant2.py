...
class RemoveEmptyStrategy:
	def __init__(self):
		self.threshold = 1.0e-6

	def find_nonemptybins(self,hists):
		sumhist = hists[0].Clone("sumHist")
                sumhist.Reset()
                for h in hists: sumhist.Add(h)
		nonempty = set()
		for ibin in range(1,sumhist.GetNbinsX()+1):
			if sumhist.GetBinContent(ibin) > self.threshold: nonempty.add(ibin)
		return nonempty

	def apply_style(self,hnew,hold):
		if hnew is None or hold is None: 
			print 'apply_style() Error: hnew or hold are None'
			print 'hnew: ', hnew, 'hold:', hold
			return
	...
	def remove(self,hists):
		newhists = []
		nonempty_bins = self.find_nonemptybins(hists)
		for h in hists:
			newhist = rt.TH1D(h.GetName()+"_nonempty","",len(nonempty_bins),0,len(nonempty_bins)-1)
			self.apply_style(newhist,h)
			newbin = 1
			for ibin in nonempty_bins:
				newhist.SetBinContent(newbin,h.GetBinContent(ibin))
				newbin += 1
...
class BinsRemover:
	...
	def remove_bins(self):
		hists = []
		if len(self.hists) > 0:
			hists = self.strategy.remove(self.hists)
		return hists
...
def main(arguments):
	...
	strat = RemoveEmptyStrategy()
	strat.threshold = 1.0e-4
	remover = BinsRemover(strat)
	...
	remover.hists = []+longhistfromfiles_pre+[longhist_rat]  #make one list with all histograms
	temp_list = remover.remove_bins()
	longhistfromfiles_pre = temp_list[0:2]
	longhist_rat = temp_list[2]
	print "Nbins:", longhistfromfiles_pre[0].GetNbinsX(), longhistfromfiles_pre[1].GetNbinsX(), longhist_rat.GetNbinsX()
	...
	leg = rt.TLegend(0.3,0.6,0.89,0.89)
	leg.SetBorderSize(0)
	leg.AddEntry(longhist_rat,'Nominal','l')
	leg.AddEntry(longhistfromfiles_pre[0],'Total {} Up'.format(arguments.sysname),'l')
	leg.AddEntry(longhistfromfiles_pre[1],'Total {} Down'.format(arguments.sysname),'l')
	...
	if arguments.ratio_reference is not None:
		...
		unitline.Draw()
		unitline.GetYaxis().SetTitle("Ratio wrt central")
	...
	else: c.Print(arguments.outfile+'_lin.'+arguments.extension)
	...
	else: c.Print(arguments.outfile+'_log.'+arguments.extension)
...
if __name__ == '__main__':
        ...
        parser.add_argument('-o', '--outfile', help="Output file", default='longhist.png')
        ...
        parser.add_argument('--sysname', help="name of the systematics added to the title", default='')
