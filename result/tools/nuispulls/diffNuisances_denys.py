...
hist_fit_b  = ROOT.TH1F("prefit_fit_b"   ,"B-only fit Nuisances;;#theta ",nbins,0,nbins)
hist_fit_s  = ROOT.TH1F("prefit_fit_s"   ,"S+B fit Nuisances   ;;#theta ",nbins,0,nbins)
hist_prefit = ROOT.TH1F("prefit_nuisancs","Prefit Nuisances    ;;#theta ",nbins,0,nbins)
...
for i in range(fpf_s.getSize()):
    ...
    for fit_name, nuis_x in [('b', nuis_b), ('s',nuis_s)]:
        if nuis_x == None:
		  ...
		  if nuis_p == None: hist_prefit.SetBinError(nuis_p_i,1000000.) # override uncertainties for rate parameters
...
if options.plotfile:
    ...
    print 'Nbins: ' + str(hist_fit_e_s.GetNbinsX())
    for b in range(1,hist_fit_e_s.GetNbinsX()+1): 
      if hist_fit_s.GetBinError(b) < 0.00001 and hist_prefit.GetBinError(b) < 0.00001:
	continue
