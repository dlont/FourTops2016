...
def main(arguments):
	...
	creator_st = OthersNorm(root_file, ['ST_tW']) # for rare split
	creator_st.set_name('ST')
	
	creator_tthz = OthersNorm(root_file, ['Rare1TTHZ']) #for rare split
	creator_tthz.set_name('Rare1TTHZ')

	creator_ttw_others = OthersNorm(root_file, ['TTRARE_plus'])
	creator_ttw_others.set_name('TTRARE_plus')

	creator_ew = OthersNorm(root_file, ['EW'])
	creator_ew.set_name('EW')
	...
	norm_prefit_st    = creator_st.get_prefit()
	# norm_prefit_st.printLatex()
	norm_prefit_ew    = creator_ew.get_prefit()
	norm_prefit_tthz    = creator_tthz.get_prefit()
	norm_prefit_ttw_others = creator_ttw_others.get_prefit()


	#make latex table
	latex_tab_header = ['SR','Obs.','$t\\bar{t}$', 'Single top', 'EW', '$t\\bar{t}$+H,Z', '$t\\bar{t}+W,XY$', '$t\\bar{t}t\\bar{t}$ \\\\']
	print ' & '.join(latex_tab_header)
	
	data.printLatex(rt.RooFit.Sibling(norm_prefit_ttbar), rt.RooFit.Sibling(norm_prefit_st), rt.RooFit.Sibling(norm_prefit_ew), rt.RooFit.Sibling(norm_prefit_tthz), 
				    rt.RooFit.Sibling(norm_prefit_ttw_others), rt.RooFit.Sibling(norm_prefit_tttt))
