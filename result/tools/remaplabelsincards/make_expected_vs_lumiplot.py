...
def main(args):
	...
	legend.AddEntry(graph_a,"combined limit (expected) SL,OS DL,SS DL","lf")
    if args.input2 != None:
	if args.label2 != None:
		legend.AddEntry(graph_a1,args.label2,"lf")
	else:
		legend.AddEntry(graph_a1,"combined limit (expected) SL, OS DL","lf")
    ...
    canvas.SaveAs("combined_limitvslumi_fwo.pdf")
