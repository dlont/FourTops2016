        ...
        for weight in weighting_scheme:     # For each weighting scheme
            ...
            for mcfile, mcset, mcName, mc_colour in mc_colour_collection: # iterate over MC processes 
                ...
                if mc_colour != 1:  # not tttt signal handling
                    ...
                    legend.AddEntry(hist_mc, mcName + " (" + "{:.0f}".format(hist_mc.Integral()) + " entries)",'lf')
                    ...
                    legend.AddEntry(hist_line, hist_line_name + " (" + "{:.0f}".format(hist_line.Integral()) + " entries)",'l')
            ...
            print "Yo!"*10
            print hist_mc_stack.GetStack().Last().GetName()
            print "Yo!"*10
            # CONTINUE HERE!!!
            ...
            gr_tot_unc = get_graph_uncertainty_band(hist_mc_stack.GetStack().Last(),hist_upline_unc,hist_dwline_unc)
            gr_tot_unc.Draw("E2 same")
            if not blind: 
                graph_data = pyr.TGraphErrors(hist_data)
                graph_data.SetMarkerStyle(20)
                graph_data.Draw("PEX0 same")
            ...
            if not blind:
                ...
                if draw_syst:
                    ...
                    hist_unc_stack.Add(hist_ratio,"PE0")

                    hist_unc_stack.Draw("nostack")
                    gr_errors.Draw("PE2")

                    hist_unc_stack.GetYaxis().SetTitle("(Data-MC)/MC ")
                    hist_unc_stack.GetYaxis().SetTitleSize(0.035)
                    # hist_unc_stack.GetYaxis().SetTitleOffset(0.55)
                    hist_unc_stack.GetYaxis().SetLabelSize(0.025)
                    # hist_unc_stack.GetYaxis().SetNdivisions(502, False)

                    hist_unc_stack.GetXaxis().SetLabelSize(0.03)
                    hist_unc_stack.GetXaxis().SetTitleSize(0.035)
                    hist_unc_stack.GetXaxis().SetTitleOffset(1.25)
                    hist_unc_stack.GetXaxis().SetTitle(propDesc)
                    if 'Number of' in hist_unc_stack.GetXaxis().GetTitle():
                        hist_unc_stack.GetXaxis().SetNdivisions(5,0,0)
                        hist_unc_stack.GetYaxis().SetDecimals(False)
                        pass
