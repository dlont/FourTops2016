...
class ObservableTH1DFromTree(Observable):
    ...
    @classmethod
    def fromnonuniform(cls, name, propNBins, list_of_bin_edges, tree=None,tree_var=None,rule=None,weight=1.):
        hist = pyr.TH1D(name, "", propNBins, list_of_bin_edges)
        return cls(name,hist,tree,tree_var,rule,weight)
...
for line in plotprops_file.readlines():
    ...
    lowbound = None
    highbound = None
    if 'uniform' in line.split()[2]:
        print line.split()[2]
        pattern = re.compile("\(.+\)")
        range = pattern.findall(str(line.split()[2]))[0]
        lowbound = float(range.split(',')[0][1:])
        highbound = float(range.split(',')[1][0:-1])
