...
//  std::string inFileName = "MVAOutput_noBWJetMass.root";
//  std::string inFileName = "MVAOutput_BWjetMass.root";
  std::string inFileName = "train_samples.root";
  TFile *inFile = new TFile(inFileName.c_str(), "OPEN");
  if (!inFile) {
      std::cerr << "Can't read input file: " << inFileName << endl;
      std::exit(1);
  }
  
  std::string singnalTreeName = "TreeS";
  TTree *TreeS = (TTree*)inFile->Get(singnalTreeName.c_str());
  if (!TreeS) {
      std::cerr << "There is no tree : " << inFileName << endl;
      std::exit(1);
  }
  std::string backgroundTreeName = "TreeB";
  TTree *TreeB = (TTree*)inFile->Get(backgroundTreeName.c_str());
  if (!TreeB) {
      std::cerr << "There is no tree : " << inFileName << endl;
      std::exit(1);
  }
...
//  std::string factoryOptions( "!V:!Silent:Transformations=I;D;P;G,D:!Color:!DrawProgressBar");
  ...
  TMVA::Factory *factory = new TMVA::Factory( OutputMVAWeights.c_str(), outFile, factoryOptions );
...
factory->AddVariable( "btag", "BTAG", "units", 'F' );
factory->AddVariable( "ThPtOverSumPt", "ThPtOverSumPt", "units", 'F' );
factory->AddVariable( "AngleThWh", "AngleThWh", "units", 'F' );
factory->AddVariable( "AngleThBh", "AngleThBh", "units", 'F' );
factory->AddVariable( "HadrWmass", "HadrWmass", "units", 'F' );
factory->AddVariable( "TopMass", "TopMass", "units", 'F' );
//factory->AddVariable( "BWjetMass", "BWjetMass", "units", 'F' );
//factory->AddVariable( "APalpha", "APalpha", "units", 'F' );
//factory->AddVariable( "APQt", "APQt", "units", 'F' );
...
//factory->AddSpectator( "spec2 := var1*3",  "Spectator 2", "units", 'F' );
  ...
  std::string CutsS, CutsB;            //Example cut => "abs(var1)<0.5 && abs(var2-0.5)<1";
  ...
  factory->PrepareTrainingAndTestTree( TCut(CutsS.c_str()), TCut(CutsB.c_str()),trainLine.c_str());
   ...
   if (Use["BDTG"]) // Gradient Boost
      factory->BookMethod( TMVA::Types::kBDT, "BDTG",
                           "!H:!V:DoBoostMonitor:NTrees=100:BoostType=Grad:MaxDepth=2:UseBaggedBoost:SeparationType=GiniIndex:nCuts=20:MinNodeSize=5:GradBaggingFraction=0.6:Shrinkage=0.30" );

   if (Use["BDTA"])  // Adaptive Boost
      factory->BookMethod( TMVA::Types::kBDT, "BDTA",
                           "!H:!V:DoBoostMonitor:NTrees=1000:BoostType=AdaBoost:MaxDepth=8:UseBaggedBoost:SeparationType=GiniIndex:nCuts=20:MinNodeSize=5:PruneMethod=NoPruning" );
  
  if (Use["BDT8TEV"])  // Adaptive Boost
