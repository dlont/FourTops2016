...
//    cutsTable->CreateTable(datasets, Luminosity);
	...
	std::string tag = GIT_TAG; 		booktup -> Branch("Tag",&tag);
	    ...
	    booktup -> Fill();
            ...
            LOG(INFO) <<"Get jets";
            selectedOrigJets                                    = r2selection.GetSelectedJets(40.,2.4,true,"Loose");                                        
                LOG(INFO) <<"Get Tight Electrons";                                                                                          
                selectedOrigElectrons                               = r2selection.GetSelectedElectrons(25, 2.5, "Tight", "Spring16_80X", true, true); 
                LOG(INFO) <<"Get Tight Muons";
                selectedMuons                                       = r2selection.GetSelectedMuons(20, 2.4, 0.15, "Tight", "Summer16"); 
	    ...
	    cout << evId << " " << init_electrons.size() << " " << init_muons.size() << " " << init_jets.size() << " " 
                 << selectedElectrons.size() << " " << selectedMuons.size() << " " << selectedJets.size() << " " << trigged << endl;
